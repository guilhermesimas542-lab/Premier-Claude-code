...
export function EntryCard({
  ...
  const tierColors: Record<string, string> = {
    ...
    pro: 'bg-[#e9b949]/20 text-[#e9b949] border-[#e9b949]/30',
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-[#e9b949]/30 bg-gradient-to-br from-[#e9b949]/20 to-[#e9b949]/10 p-4 hover:border-[#e9b949]/50 transition-colors">
      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase border ${tierBadgeClass}`}>
        {tier_required}
        {addon_required && ` + ${addon_required}`}
      </span>
      ...
      <div className="mt-3 space-y-1.5">
        {display_market && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#e9b949] font-medium">{display_market}</span>
          </div>
        )}
        {display_odd !== null && (
          <div className="flex items-center gap-2">
            <TrendingUp className="w-3 h-3 text-[#e9b949]" />
            <span className="text-xs text-white/60">Odd:</span>
            <span className="text-sm text-[#e9b949] font-bold">{display_odd.toFixed(2)}</span>
          </div>
        )}
      </div>
    </div>
