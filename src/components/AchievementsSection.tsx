...
const AchievementsSection = ({
  ...
  const AchievementButton = ({ ach, unlocked, size = 'normal' }: { ach: Achievement; unlocked: boolean; size?: 'normal' | 'small' }) => (
    <button
      ...
      style={{
        background: unlocked ? 'rgba(233,185,73,0.08)' : 'rgba(255,255,255,0.03)',
        border: unlocked ? '1px solid rgba(233,185,73,0.25)' : '1px solid rgba(255,255,255,0.06)',
    ...
    >
      ...
      <span className="text-[9px]" style={{ color: unlocked ? '#e9b949' : '#666' }}>+{ach.xp_reward} XP</span>
    </button>
  );
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div className="rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, rgba(233,185,73,0.06), rgba(0,100,0,0.08))', border: '1px solid rgba(233,185,73,0.15)' }}>
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-bold text-sm" style={{ color: '#e9b949' }}>🏆 Conquistas Permanentes</h3>
                ...
                <button key={ach.id} onClick={() => setSelectedAch(ach)} className="flex items-center gap-2 py-1 w-full text-left hover:bg-white/5 rounded px-1 transition-colors">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-bold" style={{ color: unlocked ? '#e9b949' : '#666' }}>
            ...
            {dailyAchievements.map((ach) => {
              ...
              return (
                <button
                  ...
                  className="flex flex-col items-center gap-1 p-3 rounded-xl text-center transition-all hover:scale-[1.03] active:scale-95"
                  style={{
                    background: doneToday ? 'rgba(233,185,73,0.08)' : 'rgba(0,191,255,0.05)',
                    border: doneToday ? '1px solid rgba(233,185,73,0.25)' : '1px solid rgba(0,191,255,0.1)',
                  }}
                >
                  <span className="text-xl">{ach.icon}</span>
                  {doneToday && <Check className="w-3 h-3" style={{ color: '#e9b949' }} />}
                  <span className="text-[10px] font-medium text-white leading-tight">{ach.name}</span>
                  <span className="text-[9px]" style={{ color: doneToday ? '#e9b949' : '#00BFFF' }}>+{ach.xp_reward} XP</span>
                </button>
              );
            })}
          </div>
      ...
      <Dialog open={showAllPermanent} onOpenChange={setShowAllPermanent}>
        ...
        >
          <div className="p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-base" style={{ color: '#e9b949' }}>🏆 Todas as Conquistas</h3>
              <span className="text-xs opacity-60 text-white">{unlockedPermanent.length}/{permanentAchievements.length}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
