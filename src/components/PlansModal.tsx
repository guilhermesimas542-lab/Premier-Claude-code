...
export function PlansModal({ open, onClose }: Props) {
      ...
      >
        ...
        <ul className="flex-1 space-y-2 mb-3">
          {BULLETS[plan].map((b, idx) => (
            ...
            >
              {b.available ? (
                <Check className="w-3.5 h-3.5 mt-0.5 shrink-0 text-[#10ff80]" strokeWidth={3} />
              ) : (
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="bg-[#112236] border-[#10ff80]/20 text-white w-[98vw] max-w-5xl p-3 md:p-6 rounded-2xl max-h-[90vh] overflow-y-auto [&>button]:hidden">
        <button
          onClick={onClose}
