...
export function FunnelPopup({ popup, onClose }: FunnelPopupProps) {
  ...
  // Step 0 = featured image (if exists), steps 1..N = questions, step N+1 = final
  const [step, setStep] = useState(0);

  const totalSteps = (hasImage ? 1 : 0) + questions.length + 1; // image + questions + final
  const imageStep = hasImage ? 0 : -1;
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      ...
      >
        ...
        {/* Step 0: Featured Image as button */}
        ...
        {isOnFinal && (
          <div className="p-5 space-y-4">
            <div className="text-center">
              <div
                className="w-12 h-12 rounded-full mx-auto mb-3 flex items-center justify-center bg-primary/15 border border-primary/30"
              >
