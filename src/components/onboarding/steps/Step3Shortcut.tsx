...
export function Step3Shortcut() {
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex h-full flex-col gap-4 px-5 pb-5 pt-2">
        ...
        <InstallSection
          ...
          onSimulateIab={pwa.simulateIab}
          forcedIab={pwa.forcedIab}
          onClearIabSimulation={pwa.clearIabSimulation}
        />
      </div>
...
function InstallSection({
  ...
  onSimulateIab,
  forcedIab,
  onClearIabSimulation,
}: {
  mode: "prompt" | "ios" | "iab-blocked" | "unsupported";
  installed: boolean;
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {/* Botões de simulação (dev) — só pra testar sem precisar de Android/iOS real. */}
      {(mode === "unsupported" || mode === "iab-blocked") && (
        <div className="mt-1 flex flex-wrap items-center justify-center gap-2">
          {mode === "unsupported" && (
            <button
              type="button"
              onClick={onSimulate}
              className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-white/65 hover:bg-white/10"
            >
              Simular instalación
            </button>
          )}
          {!forcedIab && mode === "unsupported" && (
            <button
              type="button"
              onClick={onSimulateIab}
              className="inline-flex items-center gap-1.5 rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-amber-200 hover:bg-amber-400/15"
            >
              Simular IAB
            </button>
          )}
        ...
        <div className="border-t border-white/[0.06] bg-[#0c1a2d] px-5 py-4">
          <ol className="space-y-2.5 text-[12.5px] leading-snug text-white/80">
            <li className="flex items-start gap-2">
              <IosStepBadge num={1} />
              <span>
                Toca el ícono <Share className="inline h-3.5 w-3.5 -translate-y-0.5" /> de
                compartir de Safari.
              </span>
            </li>
            <li className="flex items-start gap-2">
