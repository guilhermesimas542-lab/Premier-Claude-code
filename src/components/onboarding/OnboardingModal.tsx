      ...
      >
        ...
        >
          {/* `key={step}` re-monta o wrapper em cada troca → anima a entrada
              ...
              StepIndicator vai DENTRO do container scrollável para que role
              junto com o conteúdo (não fica fixo no topo). */}
          {isPopup ? (
            <>
              <StepIndicator current={step} total={total} />
              <div key={step} className="animate-popup-pop-in px-1">
                {current.render()}
              </div>
            </>
          ) : (
            <div
              key={step}
              className="flex-1 overflow-y-auto overflow-x-hidden animate-fade-in-scale"
            >
              <StepIndicator current={step} total={total} />
              {current.render()}
            </div>
