...
import { getEntries, EntriesResponse, trackEvent } from '@/lib/api';
...
export interface EntryMetadata {
  // Dados dos times (estrutura do banco)
  team1?: TeamMetadata;
  team2?: TeamMetadata;
  // Alternativa (formato antigo)
  team1_name?: string;
  team2_name?: string;
  team1_logo?: string;
  team2_logo?: string;
...
// Mapeia tier_required do backend para tier do frontend
function mapBackendTierToFrontend(
  ...
  // Múltipla se tiver mais de 1 seleção
...
// Verifica se a tip expirou (comparando com agora)
...
// Gera data de expiração default (23:59 do dia da entry)
function getDefaultExpiration(entryDate: string): string {
  return `${entryDate}T23:59:00.000Z`;
}

// Normaliza uma entry do backend para o formato do frontend
function normalizeEntry(entry: BackendEntry): DisplayEntry {
  ...
  // Extrair dados dos times (suporte para ambos os formatos)
  ...
  // Detectar tipo especial pelo título
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.entries) {
        ...
        await trackEvent('view_entries', { date: targetDate, count: normalized.length });
      } else {
