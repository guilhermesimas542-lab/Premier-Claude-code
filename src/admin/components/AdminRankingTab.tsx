...
const TIER_LABELS: Record<string, string> = { free: 'Gratis', basic: 'Básico', pro: 'Pro', ultra: 'Ultra' };
const ADDON_LABELS: Record<string, string> = { alavancagem: 'Apalancamiento', multiplas_bingo: 'Múltiples / Bingo', live_telegram: 'Live', acesso_vitalicio: 'Vitalicio' };
...
export default function AdminRankingTab() {
  ...
  const handleExportCSV = () => {
    const headers = ["Pos", "Email", "Nickname", "Nivel", "XP", "Días", "Racha", "Logros", "Último Acceso"];
  ...
  const handleCreateSpecial = async () => {
    ...
    const insertData: any = {
      id,
      name: newName,
      description: `Logro especial: ${newName}`,
      icon: newIcon,
      xp_reward: newXp,
      category: 'special',
    ...
    if (!error) {
      ...
      toast.success("¡Logro especial creado!");
      setShowCreate(false);
  ...
  const handleDeactivate = async (achId: string) => {
    ...
    toast.success("Logro desactivado");
    fetchSpecials();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} placeholder="Buscar por email o nickname..." className="pl-10 bg-gray-900 border-gray-700 text-white" />
        </div>
        <span className="text-xs text-gray-500">{xpFiltered.length} usuarios</span>
        <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10 text-left text-gray-500 text-xs">
              ...
              <SortHeader label="Usuario" sortKeyProp="email" />
              <SortHeader label="Nivel" sortKeyProp="current_level" />
              <SortHeader label="XP" sortKeyProp="total_xp" />
              <SortHeader label="Días" sortKeyProp="total_logins" />
              <SortHeader label="Racha" sortKeyProp="current_streak" />
              <SortHeader label="🏆" sortKeyProp="achievement_count" />
              <SortHeader label="Último Acceso" sortKeyProp="last_seen_at" />
          ...
          <tbody>
                ...
                <tr key={u.id} className="border-b border-white/5 text-gray-300 text-xs hover:bg-white/5">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex gap-1">
                      ...
                      <button
                        onClick={() => u.phone && copyText(u.phone, 'Teléfono')}
                        className={`p-1 rounded ${u.phone ? 'hover:bg-white/10' : 'opacity-30 cursor-not-allowed'}`}
                        title={u.phone ? 'Copiar teléfono' : 'Teléfono no registrado'}
                        disabled={!u.phone}
                      >
            ...
            {paged.length === 0 && (
              <tr><td colSpan={10} className="px-3 py-6 text-center text-gray-600">Sin datos</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          ...
          <Button size="sm" variant="outline" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="border-gray-700 bg-gray-900 text-gray-300">Siguiente</Button>
        </div>
      )}
      ...
      <div className="border-t border-white/10 pt-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-gray-300 flex items-center gap-2">
            <Trophy className="w-4 h-4 text-yellow-400" /> Logros Especiales
          </h3>
        ...
        {showCreate && (
          <div className="bg-gray-900 rounded-xl border border-white/10 p-4 space-y-3 mb-4">
            <div className="grid grid-cols-2 gap-3">
              <Input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Nombre del logro" className="bg-gray-800 border-gray-700 text-white" />
            ...
            <select value={newEntryId} onChange={e => setNewEntryId(e.target.value)} className="bg-gray-800 border border-gray-700 text-white text-sm rounded-md px-3 py-2 w-full">
              ...
              {entries.map(e => {
                ...
                const dateStr = new Date(e.date + 'T12:00:00').toLocaleDateString('es-CL');
            ...
            <Button onClick={handleCreateSpecial} disabled={creatingSpecial || !newName || !newEntryId} className="w-full">
              {creatingSpecial ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Crear Logro'}
            </Button>
          </div>
        )}
        {specials.length > 0 ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-gray-500">
                      +{s.xp_reward} XP · {s.user_count ?? 0} usuarios
                      {s.event_date && <span> · {new Date(s.event_date + 'T12:00:00').toLocaleDateString('es-CL')}</span>}
                    </div>
                  </div>
                </div>
                <Button size="sm" variant="outline" onClick={() => handleDeactivate(s.id)} className="border-red-500/30 text-red-400 hover:bg-red-500/10 text-xs">Desactivar</Button>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-xs text-gray-600 text-center py-4">Ningún logro especial activo</p>
        )}
      </div>
    </div>
  );
