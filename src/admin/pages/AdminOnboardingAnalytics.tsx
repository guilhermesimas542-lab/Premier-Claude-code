...
export default function AdminOnboardingAnalytics() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        let q = (supabase.from("fa_step_events" as any) as any)
  ...
  const { reached, firstAtBySession } = useMemo(() => {
    const reachedSets = new Map<string, Set<string>>();
    const firstAt = new Map<string, Map<string, number>>(); // session -> stepId -> ms
    events.forEach((e) => {
      if (!e.step_id || e.event_type !== "loaded") return;
    ...
    return { reached: reachedArr, firstAtBySession: firstAt };
  }, [events]);
    ...
    <div className="p-6 space-y-6 text-white">
      ...
      {loading ? (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center text-sm text-gray-500">
          Nenhum dado de onboarding no período. Os eventos começam a aparecer
          conforme os leads passam pelo funil após o deploy.
        </div>
      ) : (
        <>
          {/* KPIs */}
