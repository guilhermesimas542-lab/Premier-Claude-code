...
export default function AdminNaoAcessou() {
  ...
  const exportCSV = useCallback(() => {
    ...
    toast.success(`CSV exportado com ${filtered.length} registros`);
  }, [filtered, today]);
      ...
      <div className="rounded-lg border border-border bg-muted/10 p-4">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={chartData}>
            ...
            />
            <Bar dataKey="count" fill="#ef4444" radius={[4, 4, 0, 0]} name="Não acessou" />
          </BarChart>
        </ResponsiveContainer>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/20">
              ...
              <th className="text-left px-4 py-2 text-xs text-muted-foreground font-medium">Telefone</th>
              <th className="text-left px-4 py-2 text-xs text-muted-foreground font-medium">Plano</th>
              <th className="text-left px-4 py-2 text-xs text-muted-foreground font-medium">Liberado em</th>
