...
BEGIN
  ...
  WHERE ce.active = true
    ...
    -- Exclude entries expired more than 1 hour after starts_at
    AND (
      ce.starts_at IS NULL
      OR (ce.starts_at + interval '1 hour') > now()
    )
    -- Exclude entries past expires_at
    AND (
